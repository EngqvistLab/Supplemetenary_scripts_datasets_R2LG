...
def load_data(fname):
    ...
    for rec in SeqIO.parse(fname,'fasta'):
        ...
        # ignore data with unknown ogt
        if np.isnan(ogt): continue
    ...
    xseq = np.array(xseq)
    xogt = np.array(xogt).reshape([len(xogt),1])
    y = np.array(y).reshape([len(y),1])

    print(xseq.shape,xogt.shape,y.shape)
    
    X1_train, X1_test, X2_train, X2_test, Y_train, Y_test = make_sets2(xseq,xogt,y,split=0.9)
